...
agent: {
  ...
  provider: 'none',      // 'none' | 'stub' | 'anthropic' | 'llamacpp'
  model: '',
  endpoint: '',          // llamacpp only; empty means the loopback default
  promptVersion: 'f-1',
  consentGiven: false,
...
| --- | --- |
| `none` | Nothing. Every run fails safely and the caregiver writes the pack by hand. |
| `stub` | The F1 scripted stub. The default in tests — a real provider is always opt-in. |
| `anthropic` | A hosted model. The probe derivative leaves the machine. |
| `llamacpp` | A model running on the caregiver's own computer under `llama-server`. |

**`llamacpp` — the local option.** A vision model under 7B, 4-bit quantised, served by
llama.cpp at a loopback address. The default target is Qwen2.5-VL-3B-Instruct at Q4_K_M
with its mmproj projector; any llama.cpp-served vision model may be substituted. It must
be a *vision* model: §10.4 has the agent reading visual properties off a photograph, and a
text-only GGUF cannot do that at any size.

Two consequences follow, and both are load-bearing:

- **Privacy.** With a loopback endpoint nothing leaves the machine at all — the probe
  crosses a socket to another process on the same computer. §10.6's consent dialog says
  so, and says it *only* when the endpoint really is loopback; pointed at a LAN or public
  address it reverts to the off-machine wording.
- **Reliability.** A 3B model is not dependable at free-form function calling, so output
  is constrained at sampling time by a GBNF grammar generated from the §10.2 tool schema.
  Malformed output is therefore not rejected after the fact, it is unrepresentable — and
  because the grammar is generated from the same array that omits `commitProposal` and
  `rejectProposal`, those remain unrepresentable too (§10.7). A grammar constrains shape,
  never truth: the firewall (§10.3) still judges every proposal, unchanged.

Read tools (§10.2) are resolved into the prompt before the call rather than exposed as
callable tools, because a multi-turn read/propose loop is where a model this size loses
the thread. The model's only job is to emit proposals.
