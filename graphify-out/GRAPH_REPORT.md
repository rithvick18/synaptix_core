# Graph Report - 3D game  (2026-09-22)

## Corpus Check
- 69 files · ~112,975 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 2 file(s) not represented in the graph (top: .example 1, (none) 1)

## Summary
- 983 nodes · 2068 edges · 59 communities (41 shown, 18 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 78 edges (avg confidence: 0.81)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ed528b18`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- Missions.ts
- pack.check.ts
- layout.ts
- MissionRunner
- Three levels
- World.ts
- proceduralHouse.ts
- boot
- package.json
- createProceduralHouse
- telemetry.check.ts
- offline-check.mjs
- Player
- Telemetry
- make-photos.py
- compilerOptions
- State
- agent-prompts.check.ts
- Interaction
- agent-gemini.check.ts
- run.mjs
- PackVoices
- Proposal
- checks/tsconfig.json
- Renderer
- enabled.ts
- Door
- make-media.sh
- Illustrated portrait with dark hair, pink clothing, and letter A
- Illustration of four silhouettes around a warm light beneath an orange sky
- Illustrated portrait with dark hair bun, green clothing, and letter B
- Illustrated portrait with gray hair bun, yellow clothing, and letter R
- Illustration of a sailboat on blue water with sun and birds
- Illustrated portrait with pale hair and beard, green clothing, and letter I
- Illustrated portrait with dark hair and beard, blue clothing, and letter M
- Illustrated portrait with pale hair bun, purple clothing, and letter S
- Measured frame time
- llamaCpp.ts
- tools.ts
- ReviewSession
- LocalProfile.ts
- agent-images.check.ts
- firewall.ts
- profile-check.mjs
- review.ts
- agent-review.check.ts
- agent-provenance.check.ts
- audit.ts
- Quality.ts
- gemini.ts
- loadMedia
- main.ts
- validate
- agent-provider.check.ts
- EnvironmentEditor.ts
- Q: How do uploaded images modify the game environment?
- MemoryPack.ts
- provider.ts
- EnvironmentMaterials.ts

## God Nodes (most connected - your core abstractions)
1. `boot()` - 88 edges
2. `MissionRunner` - 33 edges
3. `UI` - 28 edges
4. `createProceduralHouse()` - 27 edges
5. `Telemetry` - 25 edges
6. `Player` - 23 edges
7. `State` - 19 edges
8. `ReviewSession` - 19 edges
9. `loadMedia()` - 17 edges
10. `three` - 15 edges

## Surprising Connections (you probably didn't know these)
- `run()` --calls--> `environmentEditor()`  [EXTRACTED]
  tools/profile-browser.check.ts → src/EnvironmentEditor.ts
- `run()` --calls--> `styleMaterial()`  [EXTRACTED]
  tools/profile-browser.check.ts → src/EnvironmentMaterials.ts
- `mutate()` --calls--> `validate()`  [EXTRACTED]
  tools/checks/pack.check.ts → src/MemoryPack.ts
- `mediaFor()` --calls--> `loadMedia()`  [EXTRACTED]
  tools/checks/pack.check.ts → src/MemoryPack.ts
- `Rig` --references--> `MissionRunner`  [EXTRACTED]
  tools/checks/pack.check.ts → src/Missions.ts

## Import Cycles
- None detected.

## Hyperedges (group relationships)
- **Three levels in one caregiver pack** — spec_memory_pack, spec_water, spec_morning_walk, spec_familiar_memories [EXTRACTED 1.00]

## Communities (59 total, 18 thin omitted)

### Community 0 - "Missions.ts"
Cohesion: 0.14
Nodes (16): applyNonLevelProposal(), AssetUrlResolver, buildPackFromProposals(), LoadedPack, ChoiceFormat, DemoNotice, FindStep, HINT_DELAYS_MS (+8 more)

### Community 1 - "pack.check.ts"
Cohesion: 0.09
Nodes (18): PackProblem, Event, failures, fakeCtx, FakeImage, fakeLoad(), FakePlayer, FakeState (+10 more)

### Community 2 - "layout.ts"
Cohesion: 0.05
Nodes (36): ARCH_HEIGHT, ARCHES, AUDIO_SOURCE_ANCHOR, BEDSIDE_FRAME_ANCHOR, BEDSIDE_FRAME_YAW, CEILING_HEIGHT, DOOR_HEIGHT, DOORS (+28 more)

### Community 3 - "MissionRunner"
Cohesion: 0.07
Nodes (8): HintBeacon, instructionOf(), MissionRunner, speak(), stopSpeaking(), Outcome, ChoiceCard, FakeUI

### Community 4 - "Three levels"
Cohesion: 0.09
Nodes (30): Deployment incomplete, Vercel and Netlify static hosting, Poly Haven runtime textures and HDRI, Smriti HTML entry, Smriti debug handle, Mira and Raju fictional demo packs, Headless and offline checks, Personalisation anchors (+22 more)

### Community 5 - "World.ts"
Cohesion: 0.14
Nodes (14): three, Focus, SwappedMaterial, StepContext, assertWorldContract(), InteractableMeta, readMeta(), REQUIRED_ANCHORS (+6 more)

### Community 6 - "proceduralHouse.ts"
Cohesion: 0.08
Nodes (27): CHAIRS, ChairSpec, FURNITURE, JUG_POSITION, LAMPS, LIVING_ROOM_WALL_YAW, OpeningSpec, SolidSpec (+19 more)

### Community 7 - "boot"
Cohesion: 0.13
Nodes (5): boot(), describe(), percentile(), escapeText(), UI

### Community 8 - "package.json"
Cohesion: 0.08
Nodes (22): dependencies, three, devDependencies, @types/node, @types/three, typescript, vite, name (+14 more)

### Community 9 - "createProceduralHouse"
Cohesion: 0.18
Nodes (18): auditDoorways(), auditReachability(), boxMesh(), buildChair(), buildFrameAnchor(), buildLamp(), buildPlant(), buildProp() (+10 more)

### Community 10 - "telemetry.check.ts"
Cohesion: 0.05
Nodes (28): buildExport(), downloadJson(), DWELL_THRESHOLD_MS, dwellByObject(), DwellTotal, EventListener, ExportContext, ExportDocument (+20 more)

### Community 11 - "offline-check.mjs"
Cohesion: 0.10
Nodes (13): args, CDP, chrome, consoleErrors, failures, originArg, pageErrors, PERF (+5 more)

### Community 14 - "make-photos.py"
Cohesion: 0.29
Nodes (16): band(), blank(), disc(), ellipse(), glyph(), grain(), lerp(), over() (+8 more)

### Community 15 - "compilerOptions"
Cohesion: 0.12
Nodes (16): compilerOptions, allowImportingTsExtensions, isolatedModules, lib, module, moduleResolution, noEmit, noFallthroughCasesInSwitch (+8 more)

### Community 17 - "agent-prompts.check.ts"
Cohesion: 0.13
Nodes (19): anchorLines(), assetLines(), AUTHORING_REASONING, AuthoringContext, buildAuthoringSystemPrompt(), bullets(), ENVIRONMENT_REASONING, ENVIRONMENT_SYSTEM_PROMPT (+11 more)

### Community 19 - "agent-gemini.check.ts"
Cohesion: 0.09
Nodes (28): AgentConfig, agentConfigStore, AgentProvider, applySetupMode(), ConsentPrompt, consentPromptFor(), defaultModelForMode(), ensureConsent() (+20 more)

### Community 20 - "run.mjs"
Cohesion: 0.22
Nodes (8): checks, esbuild, filter, here, out, root, three, types

### Community 22 - "Proposal"
Cohesion: 0.33
Nodes (3): FirewallContext, ProposalFixture, Proposal

### Community 23 - "checks/tsconfig.json"
Cohesion: 0.29
Nodes (6): ../../tsconfig.json, compilerOptions, noEmit, types, extends, include

### Community 37 - "llamaCpp.ts"
Cohesion: 0.21
Nodes (14): classifyStatus(), classifyThrown(), friendlyMessage(), imageBlock(), isLoopback(), isRetryable(), LlamaCppProviderAdapter, ParsedEnvelope (+6 more)

### Community 38 - "tools.ts"
Cohesion: 0.06
Nodes (53): AuthoringRun, construct(), matchesType(), missingArgument(), runAuthoringPass(), SkippedCall, toProposals(), DEFAULT_AGENT_CONFIG (+45 more)

### Community 39 - "ReviewSession"
Cohesion: 0.30
Nodes (4): ReviewSession, ProposalReviewList, renderProvenanceSummary(), CaregiverInputRequest

### Community 40 - "LocalProfile.ts"
Cohesion: 0.19
Nodes (21): EnvironmentStyle, Crop, LocalPerson, LocalProfile, newId(), newProfile(), openDatabase(), Photo (+13 more)

### Community 41 - "agent-images.check.ts"
Cohesion: 0.11
Nodes (16): ACCEPTED_MIME, dimensionsFor(), encode(), ImageDerivatives, ImagePipelineError, ImagePipelineErrorReason, nextPowerOfTwo(), receiveImage() (+8 more)

### Community 42 - "firewall.ts"
Cohesion: 0.17
Nodes (18): AssetRegistry, checkClinicalVocabulary(), checkHedgingAndForm(), checkHighlightTarget(), checkHints(), checkText(), checkTokensAgainstAllowlist(), CLINICAL_DENYLIST (+10 more)

### Community 43 - "profile-check.mjs"
Cohesion: 0.13
Nodes (10): /src/LocalProfile.ts, /tools/profile-browser.check.ts, args, CDP, chrome, originArg, profile, REPO (+2 more)

### Community 44 - "review.ts"
Cohesion: 0.22
Nodes (10): Violation, ProposalStatus, ReviewCounts, ReviewedProposal, CropHandles, statusLabel(), summarise(), violationLine() (+2 more)

### Community 45 - "agent-review.check.ts"
Cohesion: 0.16
Nodes (11): assets(), ctx(), emptyPack, failures, fakeCtx, FakeImage, fakeLoad(), firewallWorld() (+3 more)

### Community 46 - "agent-provenance.check.ts"
Cohesion: 0.21
Nodes (7): computeProvenance(), ProvenanceBlock, buildAllowedTokens(), CaregiverInputs, words(), ctx(), failures

### Community 48 - "audit.ts"
Cohesion: 0.22
Nodes (4): AuditEntry, AuditLog, AuditSink, consoleAuditSink

### Community 49 - "Quality.ts"
Cohesion: 0.11
Nodes (16): AdaptiveOptions, AdaptiveResolution, AdaptiveStep, clampRatio(), detectQuality(), detectTier(), DeviceInfo, isSmall() (+8 more)

### Community 50 - "gemini.ts"
Cohesion: 0.19
Nodes (15): ALLOWED_HOSTS, classifyStatus(), classifyThrown(), friendlyMessage(), GeminiProviderAdapter, imagePart(), InteractionStep, isGoogleEndpoint() (+7 more)

### Community 51 - "loadMedia"
Cohesion: 0.23
Nodes (7): loadAudio(), loadImage(), loadMedia(), loadTexture(), PackMedia, placeholderTexture(), mediaFor()

### Community 52 - "main.ts"
Cohesion: 0.12
Nodes (16): FocusProbe, PerfResult, breakagesFromLocation(), patientIdFromLocation(), QualityProfile, GameState, MOVEMENT_ENABLED, POINTER_LOCK_WANTED (+8 more)

### Community 53 - "validate"
Cohesion: 0.24
Nodes (7): isObject(), loadPack(), PackRejected, Problems, resolvePackPath(), validate(), validateStep()

### Community 54 - "agent-provider.check.ts"
Cohesion: 0.12
Nodes (12): CONSENT_PROMPT, DEFAULT_GEMINI_MODEL, GeminiConfig, LlamaCppConfig, EMPTY_STUB_SCRIPT, NullProviderAdapter, SelectProviderOptions, StubProviderAdapter (+4 more)

### Community 55 - "EnvironmentEditor.ts"
Cohesion: 0.50
Nodes (7): needsSetup(), geminiConfigFromEnv(), llamaCppConfigFromEnv(), selectProvider(), destinationLine(), environmentEditor(), waitingLine()

### Community 56 - "Q: How do uploaded images modify the game environment?"
Cohesion: 0.40
Nodes (4): Answer, Outcome, Q: How do uploaded images modify the game environment?, Source Nodes

### Community 57 - "MemoryPack.ts"
Cohesion: 0.22
Nodes (10): CardStyle, fitToPlate(), injectAnchors(), Json, LoadOptions, MediaOptions, plateOf(), REDUCE_SENTINEL (+2 more)

### Community 58 - "provider.ts"
Cohesion: 0.15
Nodes (14): colors, describeEnvironment(), validateEnvironment(), JsonSchemaToolList, ProbeImage, ProviderAdapter, ProviderFailure, ProviderFailureReason (+6 more)

### Community 59 - "EnvironmentMaterials.ts"
Cohesion: 0.31
Nodes (6): colourFor(), styleAnisotropy(), styleMaterial(), stylesSurface(), Surface, Materials

## Knowledge Gaps
- **218 isolated node(s):** `name`, `private`, `version`, `type`, `dev` (+213 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 354 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **18 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `boot()` connect `boot` to `MissionRunner`, `World.ts`, `LocalProfile.ts`, `createProceduralHouse`, `telemetry.check.ts`, `Player`, `Telemetry`, `State`, `Quality.ts`, `Interaction`, `agent-gemini.check.ts`, `main.ts`, `loadMedia`, `validate`, `EnvironmentEditor.ts`, `Renderer`, `MemoryPack.ts`, `PackVoices`?**
  _High betweenness centrality (0.082) - this node is a cross-community bridge._
- **Why does `three` connect `World.ts` to `Missions.ts`, `pack.check.ts`, `layout.ts`, `proceduralHouse.ts`, `package.json`, `LocalProfile.ts`, `telemetry.check.ts`, `agent-review.check.ts`, `Quality.ts`, `main.ts`, `MemoryPack.ts`, `provider.ts`, `EnvironmentMaterials.ts`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Why does `MissionRunner` connect `MissionRunner` to `Missions.ts`, `pack.check.ts`, `boot`, `telemetry.check.ts`, `main.ts`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Are the 49 inferred relationships involving `boot()` (e.g. with `.activate()` and `.clear()`) actually correct?**
  _`boot()` has 49 INFERRED edges - model-reasoned connections that need verification._
- **What connects `name`, `private`, `version` to the rest of the system?**
  _218 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Missions.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.1437908496732026 - nodes in this community are weakly interconnected._
- **Should `pack.check.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.08817204301075268 - nodes in this community are weakly interconnected._