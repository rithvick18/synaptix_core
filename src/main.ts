...
import { MissionRunner, stopSpeaking, type MemoryPack, type Mission } from './Missions'
// The only static import from `src/agent/`. It has no imports of its own, so the agent
// layer stays out of the bundle until the dynamic import in `openCaregiverSetup` runs.
...
async function boot(): Promise<void> {
  ...
  /**
   * Folds an agent-assisted pack into the running session (SPEC.md §10 F3/F4).
   *
   * It goes through §4.2's `validate()` and §4.1's `loadMedia()` exactly as a pack
   * fetched from disk does — the pack format is not forked for agent-authored content,
   * so this path cannot accept anything the normal one would reject. A fatal problem
   * leaves the session untouched.
   */
  const applyAgentPack = async (nextPack: MemoryPack): Promise<void> => {
    const { pack: revalidated, problems } = validate(nextPack, world, true)
    for (const problem of problems) {
      console.warn(`[smriti] agent pack · ${problem.where} · ${problem.message}`)
    }
    if (!revalidated) return
    const { media } = await loadMedia(revalidated)
    injectAnchors(world, media)
    voices.use(media)
    for (const mission of revalidated.missions) {
      if (!levels.some((level) => level.id === mission.id)) levels.push(mission)
    }
  }

  /**
   * §10's caregiver setup screen, behind a dynamic import: with the feature off, a
   * browser never downloads the agent layer at all. `enabled.ts` is the only part of it
   * on the static boot path, and it imports nothing itself.
   */
  const openCaregiverSetup = async (): Promise<void> => {
    player.releaseLock()
    ui.hideOverlay()
    overlayMode = 'none'
    const [setup, config] = await Promise.all([
      import('./agent/caregiverSetup'),
      import('./agent/config')
    ])
    setup.openCaregiverSetup({
      container: document.body,
      world: setup.worldFactsFromSource(world),
      basePack: pack,
      config: config.agentConfigStore.load(),
      saveConfig: (next) => config.agentConfigStore.save(next),
      onCommit: (nextPack) => void applyAgentPack(nextPack),
      onClose: () => showLevels()
    })
  }
  ...
  const showLevels = (): void => {
    ...
    ui.showLevelSelect({
      ...
      // SPEC.md §10. Undefined unless a caregiver turned the feature on in this browser,
      // so a shipped build renders no button and never loads the module behind it.
      onCaregiverSetup: isAgentEnabled() ? () => void openCaregiverSetup() : undefined,
      storageWarning,
