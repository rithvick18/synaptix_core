...
export function openProfileEditor(saved: LocalProfile | undefined, maxTextureSize: number, onClose: () => void): void {
  ...
  <h2>Personalise Home</h2><p>Your profiles and original photographs stay in this browser on this device. Personal media is never uploaded. Clearing browser data removes them.</p>
  ...
  async function run(action: () => Promise<void>): Promise<void> {
    ...
    dialog.querySelectorAll<HTMLInputElement>('button,input,select').forEach(b => { b.disabled = true })
    try { await action() } catch (e) { message(e) }
    finally { busy = false; dialog.querySelectorAll<HTMLInputElement>('button,input,select').forEach(b => { b.disabled = false }) }
  }
